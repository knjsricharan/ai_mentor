...
const ChatView = ({ projectId, project }) => {
  ...
  useEffect(() => {
    ...
    firestoreUnsubscribeRef.current = loadChatMessages(projectId, (firestoreMessages) => {
      ...
      if (validMessages.length > 0) {
        // We have messages from Firestore, use them
        setMessages(validMessages);
        setIsLoadingHistory(false);
      } else if (!initialMessageShownRef.current && hasLoadedOnceRef.current) {
        // No messages in Firestore and we haven't shown initial message yet
        // Only show initial message after we've confirmed Firestore is empty
        ...
        // Save initial message to Firestore (but don't wait for it)
        saveChatMessage(projectId, 'model', content).catch(err => {
          console.error('Failed to save initial message to Firestore:', err);
        });
      } else if (hasLoadedOnceRef.current && validMessages.length === 0) {
        // Firestore confirmed empty, but we've already shown initial message
        setIsLoadingHistory(false);
      }
    });
  ...
  const handleSend = async (e) => {
    ...
    setMessages(prev => {
      ...
      // Build conversation history from updated messages (including the new user message)
      // Exclude the initial message and ensure we have valid messages
      const conversationHistory = updatedMessages
        .filter(msg => msg && msg.id !== 'initial' && msg.role && msg.content)
        .map(msg => ({
      ...
      generateAIResponse(userInput, conversationHistory, project || {})
        .then(aiResponse => {
          const aiMessage = {
